...
class Yolov3LogVisualization:
    ...
    def parse_loss_log(self, log_path, line_num=26800):
        result = pd.read_csv(log_path, skiprows=[x for x in range(line_num) if ((x % 10 != 9) | (x < 1000))],
                             error_bad_lines=False, names=['loss', 'avg', 'rate', 'seconds', 'images'])
        #result = pd.read_csv(log_path, error_bad_lines=False, names=['loss', 'avg', 'rate', 'seconds', 'images'])
    ...
    def parse_iou_log(self, log_path, line_num=26800):
